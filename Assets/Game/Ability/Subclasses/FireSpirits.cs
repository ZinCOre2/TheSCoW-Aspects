...
public class FireSpirits : Ability
{
    public override void UseAbility(Unit user, List<PathNode> aoe)
    {
        ...
        foreach (var pathNode in aoe)
        {
            ...
            var aEffect = GameController.Instance.ObjectPooler.SpawnFromPool(abilityEffect.EffectTag, pathNode.node.transform.position, abilityEffect.transform.rotation).GetComponent<AbilityEffect>();
